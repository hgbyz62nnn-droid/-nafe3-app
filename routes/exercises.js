...
const { clampStr } = require('../lib/sanitize');
...
router.get('/', requireAuth, requireRole('coach'), (req, res) => {
  ...
  const exercises = db
    .prepare(
      `SELECT e.id, e.name, e.muscle_group, e.equipment, e.difficulty, e.video_url, e.coach_id,
...
router.post('/', requireAuth, requireRole('coach'), (req, res) => {
  ...
  const info = db
    .prepare('INSERT INTO exercises (coach_id, name, muscle_group, equipment, difficulty, video_url) VALUES (?, ?, ?, ?, ?, ?)')
    .run(req.user.id, name, muscleGroup, equipment, difficulty, videoUrl || null);
